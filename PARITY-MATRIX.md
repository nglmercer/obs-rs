...
The current dock and scene-properties UI intentionally continues to submit
left until its direction selector is implemented.
...
| STUDIO-002 | Transition catalog | Per-scene transitions, overrides, duration, stinger, swipe, slide, color, and luma transitions behave like OBS. | Cut and timed cross-fade/reference fade behavior now run through the single worker with a validated 1–60,000 ms duration. A bounded CPU/reference Fade to Color transition covers source → configured RGBA color → destination. Slide moves both layers without a second frame allocation; Swipe moves the source while the destination stays fixed and fills the revealed area. Both persist through typed `TransitionSpec` kinds and all four reference directions, and the console accepts legacy `transition/take slide|swipe <progress>` plus explicit `... <direction> <progress>` forms. The Transition dock and scene-properties dialog still submit the left default. A shared validated `TransitionSpec` persists optional destination-scene overrides in schema v7, migrates older documents to inheritance, and applies the override through typed project/UI commands. Stinger, luma, `swipe_in`, and visual direction selection remain absent. | Partial | Portable/desktop | Media Slide/Swipe correctness for all four directions, ignored 640x360 Slide/Swipe timing reports, project JSON direction round-trips, UI console parser coverage, and existing GUI left-direction workflow fixtures. Reference: [OBS 32.2.2 transition plugin sources](https://github.com/obsproject/obs-studio/tree/32.2.2/plugins/obs-transitions). | `crates/obs-rs-media/src/{transition.rs,frame_transitions.rs,media_tests_transitions.rs}`, `crates/obs-rs-project/src/{codec.rs,project_tests_round_trip.rs}`, `crates/obs-rs-ui/src/{console.rs,ui_tests_persistence.rs}`, `crates/obs-rs-gui/src/{callbacks/{output.rs,project.rs},refresh.rs,tests/{runtime.rs,ui_output.rs}}`, `crates/obs-rs-gui/ui/{dialogs.slint,transition_dock.slint}` | GUI direction selector, full transition catalog/assets/plugins, audio transition policy |
