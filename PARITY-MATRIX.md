...
## Latest verified package: Slide and Swipe transitions
...
the timing reports remain ignored benchmarks. Stinger, luma, arbitrary swipe
directions, `swipe_in`, and other slide variants remain incomplete.
...
| STUDIO-002 | Transition catalog | Per-scene transitions, overrides, duration, stinger, swipe, slide, color, and luma transitions behave like OBS. | Cut and timed cross-fade/reference fade behavior now run through the single worker with a validated 1–60,000 ms duration. A bounded CPU/reference Fade to Color transition covers source → configured RGBA color → destination. The bounded Slide-from-left transition moves both layers without a second frame allocation; the bounded Swipe-from-left transition moves the source while the destination stays fixed and fills the revealed area. Both persist through typed `TransitionSpec` kinds/directions, are exposed by the Transition dock and scene-properties dialog, and are available through typed console `transition/take slide|swipe <progress>` commands. A shared validated `TransitionSpec` persists optional destination-scene overrides in schema v7, migrates older documents to inheritance, and applies the override through typed project/UI commands. Stinger, luma, arbitrary swipe directions, `swipe_in`, and other slide variants remain absent. | Partial | Portable/desktop | Media Slide/Swipe correctness and policy tests plus ignored 640x360 Slide/Swipe timing reports; project JSON kind/direction round-trips; UI console parser and transient-state coverage; GUI callback, scene-override, localized-label, and preview/output workflow fixtures. Reference: [OBS 32.2.2 transition plugin sources](https://github.com/obsproject/obs-studio/tree/32.2.2/plugins/obs-transitions). | `crates/obs-rs-media/src/{transition.rs,frame.rs,frame_transitions.rs,media_tests_transitions.rs}`, `crates/obs-rs-project/src/{codec.rs,project_tests_round_trip.rs}`, `crates/obs-rs-ui/src/{commands.rs,console.rs,ui_tests_persistence.rs}`, `crates/obs-rs-gui/src/{callbacks/{output.rs,project.rs},refresh.rs,i18n/{english.rs,spanish.rs},tests/{runtime.rs,ui_output.rs}}`, `crates/obs-rs-gui/ui/{dialogs.slint,i18n.slint,main.slint,transition_dock.slint,dock_slot.slint,dock_workspace.slint,floating_dock.slint}` | Full transition catalog/assets/plugins, audio transition policy |
