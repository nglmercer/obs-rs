...
| STUDIO-002 | Transition catalog | Per-scene transitions, overrides, duration, stinger, swipe, slide, color, and luma transitions behave like OBS. | Cut and timed cross-fade/reference fade behavior now run through the single worker with a validated 1–60,000 ms duration. A bounded CPU/reference Fade to Color transition covers source → configured RGBA color → destination. Slide moves both layers without a second frame allocation; outgoing Swipe moves the source while the destination stays fixed, and Swipe In moves the destination over the stationary source without a second frame allocation. Both persist through typed `TransitionSpec` kinds and all four reference directions, and the console accepts legacy `transition/take slide|swipe <progress>` plus explicit direction/mode forms. The docked and floating Transition panels plus the scene-properties dialog expose localized Left/Right/Up/Down and Swipe In controls and pass them through typed callbacks and persisted scene overrides. A shared validated `TransitionSpec` persists optional destination-scene overrides in schema v7, migrates older documents to inheritance, and applies the override through typed project/UI commands. Stinger, luma, and the remaining transition catalog/assets/plugins are still absent. | Partial | Portable/desktop | Media Slide/Swipe correctness for all four directions and Swipe In, ignored 640x360 Slide/Swipe timing reports, project JSON direction/mode round-trips, UI console parser coverage, runtime-label coverage, and GUI fixtures covering left-compatible plus right/up/Swipe In workflows. Reference: [OBS 32.2.2 transition plugin sources](https://github.com/obsproject/obs-studio/tree/32.2.2/plugins/obs-transitions). | `crates/obs-rs-media/src/{transition.rs,frame_transitions.rs,media_tests_transitions.rs}`, `crates/obs-rs-project/src/{codec.rs,project_tests_round_trip.rs}`, `crates/obs-rs-ui/src/{console.rs,commands.rs,ui_tests_persistence.rs}`, `crates/obs-rs-gui/src/{callbacks/{output.rs,project.rs,docks.rs,docks_forward.rs},refresh.rs,tests/{runtime.rs,ui_output.rs},callbacks/scene_tests.rs,i18n/{english.rs,spanish.rs}}`, `crates/obs-rs-gui/ui/{dialogs.slint,transition_direction.slint,transition_dock.slint,dock_slot.slint,dock_workspace.slint,floating_dock.slint,main.slint,main_modals.slint,i18n.slint}` | Stinger, luma, other transition assets/plugins, audio transition policy |
