...
| GAP-010 | Audio is a reference mixer, not a full graph | Mixer, resampler, monitor taps, pacing, PipeWire discovery, deterministic fallback, transient active-device identity, and bounded recovery for configured and automatic microphone/desktop routes exist; full device graph, default-device policy, and multiple tracks do not. | Advanced audio workflows and production synchronization are incomplete. | Model device/source/filter/gain/pan/bus/track/monitor routing with bounded blocks and clock ownership; keep device recovery off UI paths and preserve explicit fallback state. | Work 019; Audio Agent, then platform agents. |
