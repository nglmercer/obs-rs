...
| GAP-015 | Settings and hotkeys are not fully typed at the behavior boundary | Settings persist several output fields and hotkey display strings; the toolkit-neutral `Shortcut` parser now validates bounded Ctrl/Shift/Alt combinations, canonicalizes aliases/order, and preserves empty unbindings before settings persistence. | Conflicts, global registration, runtime effects, and restart requirements are still hard to validate centrally; the GUI event path still compares canonical text. | Structured setting schema and `KeyCombination` model with capability, validation, effect, and restart metadata, followed by runtime/platform registration. | Work 025–026; UI/Platform Agents. |
