...
| GAP-010 | Audio is a reference mixer, not a full graph | Mixer, resampler, monitor taps, pacing, PipeWire discovery, deterministic fallback, transient active-device identity, bounded recovery for configured and automatic microphone/desktop routes, scheduled A/V drift counters, and a sample-reusing positive per-channel sync-delay boundary exist. Full device graph, default-device policy, adaptive hardware clock correction, monitor modes, persistent offset controls, and multiple tracks do not. | Advanced audio workflows and production synchronization are incomplete; the current drift counters describe the portable scheduler rather than measured independent device clocks, and the new delay control is not yet projected into settings or persisted source properties. | Model device/source/filter/gain/pan/bus/track/monitor routing with bounded blocks and clock ownership; add settings/UI sync-offset projection, measured clock feedback, adaptive resampling, and monitor/track routing without unbounded latency; keep device recovery off UI paths and preserve explicit fallback state. | Work 019; Audio Agent, then UI and platform agents. |
