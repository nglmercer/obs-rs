...
| GAP-010 | Audio is a reference mixer, not a full graph | Mixer, resampler, monitor taps, pacing, PipeWire discovery, deterministic fallback, transient active-device identity, bounded recovery for configured and automatic microphone/desktop routes, scheduled A/V drift counters, a sample-reusing positive per-channel sync-delay boundary, and validated/persisted global microphone/desktop offset controls projected through the Audio settings page exist. Full device graph, default-device policy, adaptive hardware clock correction, monitor modes, source-level sync properties, and multiple tracks do not. | Advanced audio workflows and production synchronization are incomplete; the current drift counters describe the portable scheduler rather than measured independent device clocks, while settings currently control only positive global channel delay. | Model device/source/filter/gain/pan/bus/track/monitor routing with bounded blocks and clock ownership; add measured clock feedback, adaptive resampling, monitor modes, channel-layout/source-property routing, and multiple tracks without unbounded latency; keep device recovery off UI paths and preserve explicit fallback state. | Work 019; Audio Agent, then UI and platform agents. |
