...
| GAP-006 | Canvas interaction state is implicit in UI properties | `CanvasState` now owns bounded transient zoom and pan; selected item rectangle and drag draft remain separate callback/UI fields. Snapping, multi-selection, and keyboard state are still absent. | New canvas features can duplicate state or accidentally persist pointer drafts; wheel zoom is still preset-stepped rather than cursor-anchored. | Extend the toolkit-neutral `CanvasState { zoom, pan, selection, snapping, interaction }`; keep drafts transient and commit one command per gesture. | Work 010 onward; Canvas Agent. |
