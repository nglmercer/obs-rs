...
| GAP-015 | Settings and hotkeys are not fully typed at the behavior boundary | Settings persist several output fields and hotkey display strings; the toolkit-neutral `Shortcut` parser now validates bounded Ctrl/Shift/Alt combinations, canonicalizes aliases/order, preserves empty unbindings, and Settings Apply rejects canonical duplicate local bindings with a visible conflict warning. Undo, Redo, Save project, and Fade transition now use the same validated settings path, including persistence, menu labels where applicable, and GUI runtime delivery. | Global registration, runtime effects, restart requirements, and the remaining action set are still hard to validate centrally; the GUI event path still compares canonical text. | Structured setting schema and `KeyCombination` model with capability, validation, effect, and restart metadata, followed by runtime/platform registration. | Work 025–026; UI/Platform Agents. |
