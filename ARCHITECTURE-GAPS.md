...
| GAP-008 | Dock interaction and floating geometry are incomplete | `DockNode::Split`, `DockNode::Tabs`, and `DockNode::Dock` provide one bounded, validated tree with versioned persistence; Rust emits pane rectangles and the Slint workspace renders split regions and tab activation. | Drag hit-testing, insertion indicators, split-handle resizing, per-window geometry, and monitor/DPI restoration remain unavailable. | Complete drag insertion/mutation, add bounded split handles, persist floating geometry/monitor metadata, and retain legacy migration. | Work 014–015; Dock Agent. |
