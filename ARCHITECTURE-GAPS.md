...
| GAP-010 | Audio is a reference mixer, not a full graph | Mixer, resampler, monitor taps, pacing, PipeWire discovery, deterministic fallback, transient active-device identity, bounded recovery for configured and automatic microphone/desktop routes, scheduled A/V drift counters, a sample-reusing positive per-channel sync-delay boundary, validated/persisted global microphone/desktop offset controls projected through the Audio settings page, and typed per-source `AudioMonitorMode` output/monitor bus routing now exist. Full device graph, default-device policy, adaptive hardware clock correction, asynchronous monitor sink delivery, source-level sync properties, channel-layout routing, and multiple tracks do not. | Advanced audio workflows and production synchronization are incomplete; the current drift counters describe the portable scheduler rather than measured independent device clocks, settings currently control only positive global channel delay, and monitor-mode state is not yet exposed through the engine/worker or connected to a hardware sink. | Model device/source/filter/gain/pan/bus/track/monitor routing with bounded blocks and clock ownership; connect the monitor bus to a dedicated bounded output worker, add measured clock feedback and adaptive resampling, then expose monitor modes, channel-layout/source-property routing, and multiple tracks without unbounded latency; keep device recovery off UI paths and preserve explicit fallback state. | Work 019; Audio Agent, then UI and platform agents. |
