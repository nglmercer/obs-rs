...
| GAP-008 | Dock presentation is still a flat-row adapter | `DockNode::Split`, `DockNode::Tabs`, and `DockNode::Dock` now provide one bounded, validated tree with a compact persisted representation; legacy order/weight settings still migrate into it. | The visible Slint row still renders parallel models, so tabs, nested split interaction, insertion indicators, per-window geometry, and monitor/DPI restoration remain unavailable. | Render and mutate the tree directly, add drag insertion targets, persist floating geometry/monitor metadata, and retain legacy migration. | Work 014–015; Dock Agent. |
