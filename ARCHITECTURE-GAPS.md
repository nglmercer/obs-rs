...
| GAP-008 | Dock interaction and floating geometry are incomplete | `DockNode::Split`, `DockNode::Tabs`, and `DockNode::Dock` provide one bounded, validated tree with versioned persistence; Rust emits pane and splitter projections, header drags resolve tab/directional targets, detached docks restore scale-aware physical geometry, and projector windows persist bounded physical geometry plus fullscreen state through a versioned record, the same desktop clamp, and DPI scaling helpers. The existing monitor capability supplies a virtual-desktop clamp for restored positions, while platforms without monitor bounds retain the saved coordinates. | OS monitor identity, per-monitor selection, automatic projector reopening, and compositor-backed multi-monitor/DPI evidence are still absent; floating custom-dock/plugin surfaces are not yet supported. | Add platform monitor identity/capability adapters, projector monitor selection/reopening lifecycle, and live multi-monitor fixtures while retaining the shared bounded geometry/clamp helper and legacy migration. | Work 014–015 and projector persistence follow-up; Dock Agent, Platform Agents, QA Agent. |
