...
demand state, fan-out without duplicate source pulls, and complete allocation
profiling. These are intentionally still open; passing a viewport test is not
the same as zero-copy or final performance certification.
...
| GAP-006 | Canvas interaction state is implicit in UI properties | `CanvasState` owns bounded transient zoom, pan, and fixed-policy snapping; crop/rotation remain in the typed transform draft and are committed once per gesture. Selection is still single-item and keyboard state is absent. | New canvas features can duplicate state or accidentally persist pointer drafts; wheel zoom is still preset-stepped rather than cursor-anchored and crop handles do not yet support rotated geometry. | Extend the toolkit-neutral `CanvasState { zoom, pan, selection, snapping, interaction }`; keep drafts transient and commit one command per gesture. | Work 009–013; Canvas Agent. |
