...
| GAP-010 | Audio is a reference mixer, not a full graph | Mixer, resampler, monitor taps, pacing, PipeWire discovery, deterministic fallback, transient active-device identity, and bounded recovery for configured and automatic microphone/desktop routes exist; engine stats and GUI diagnostics also expose bounded scheduled A/V drift counters. Full device graph, default-device policy, adaptive hardware clock correction, and multiple tracks do not. | Advanced audio workflows and production synchronization are incomplete; the current drift counters describe the portable scheduler rather than measured independent device clocks. | Model device/source/filter/gain/pan/bus/track/monitor routing with bounded blocks and clock ownership; add measured clock feedback and adaptive resampling without unbounded latency; keep device recovery off UI paths and preserve explicit fallback state. | Work 019; Audio Agent, then platform agents. |
