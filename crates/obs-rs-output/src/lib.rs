...
const WEBSOCKET_PACKET_MAGIC: &[u8; 8] = b"OBSRWS01";
...
///
/// The endpoint accepts `ws://host:port/path` (or `host:port/path`) and performs
/// the HTTP upgrade handshake before sending masked binary frames. TLS is
/// deliberately not guessed or silently downgraded; `wss://` is rejected until a
/// reviewed Rust TLS boundary is selected. Packet payloads use an explicit
/// `OBSRWS01` envelope inside each WebSocket message so a receiver can validate
/// media kind, timestamp, keyframe state, and byte bounds before decoding.
...
fn parse_websocket_endpoint(endpoint: &str) -> Result<(String, String, String), OutputError> {
...
fn read_websocket_headers(stream: &mut TcpStream) -> Result<String, OutputError> {
...
fn websocket_packet_body(packet: &EncodedPacket) -> Result<Vec<u8>, OutputError> {
