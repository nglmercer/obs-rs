...
impl OutputRuntime {
    ...
    pub(crate) fn output_status(&self) -> String {
        ...
        let protocols = self
            ...
            .collect::<Vec<_>>()
            .join("/");
        let production_error = engine
            .production_stream_error
            .as_deref()
            .map_or_else(String::new, |error| format!(" · stream error {error}"));
        format!(
            "Output: recording {recording} · stream {streaming}{production_error} · replay {} ({} packets) · replay save {replay_save} · {audio} · {monitor} · {worker} · available {protocols}",
    ...
    pub(crate) fn output_metrics(&self) -> String {
        ...
        format!(
            ...
            monitor_dropped,
            monitor_reconnects,
            engine.production_stream_error.as_deref().unwrap_or("none"),
    ...
    pub(crate) fn diagnostics_document(&mut self) -> String {
        ...
        let replay_save = replay_save_label(&engine.replay_save_status);
        let capability_status = self.capabilities.production_status();
        let capability_detail = self.capabilities.production_status_detail();
        ...
        format!(
            ...
            engine.audio_active_device_id.as_deref().unwrap_or("none"),
