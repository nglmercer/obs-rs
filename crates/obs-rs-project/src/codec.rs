...
fn encode_filter(filter: &FrameFilter) -> Json {
    match filter {
        ...
        FrameFilter::Brightness { milli } => Json::object([
            ("kind", Json::string("brightness")),
            ("milli", Json::number(milli)),
        ]),
        FrameFilter::Opacity(opacity) => Json::object([
            ("kind", Json::string("opacity")),
            ("value", Json::number(opacity)),
        ]),
    }
}
...
fn decode_source(value: &Json) -> Result<SourceSpec, ProjectError> {
    ...
    for filter in array_member(value, "filters")? {
        source.add_filter(decode_filter(filter)?);
    }
...
fn decode_filter(value: &Json) -> Result<FrameFilter, ProjectError> {
    match string_member(value, "kind")? {
        "grayscale" => Ok(FrameFilter::Grayscale),
        "brightness" => Ok(FrameFilter::Brightness {
            milli: number_member(value, "milli")?,
        }),
        "opacity" => Ok(FrameFilter::Opacity(number_member(value, "value")?)),
