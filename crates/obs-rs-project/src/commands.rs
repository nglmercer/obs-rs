...
use groups::{
    ...
    paste_group_item, remove_group_item, remove_scene_item_target, remove_scene_items,
...
use source_definitions::{duplicate_source, remove_source, set_source_name, set_source_settings};
    ...
    pub fn apply(&mut self, command: ProjectCommand) -> Result<(), ProjectError> {
        match command {
            ...
            ProjectCommand::RemoveSceneItem {
                ...
                if item.contains('/') {
                    remove_scene_item_target(self, &profile, &scene, &item)
                } else {
